...
## Точечные Фиксы После Разбиения

- WB accrual: `Выручка с учетом СПП` теперь считается атомарно как `SUM("Цена розничная с учетом согласованной скидки")` по строкам `Продажа` минус `SUM("Цена розничная с учетом согласованной скидки")` по строкам `Возврат`. Зависимые метрики используют исправленный `revenueBeforeSpp`.
- WB accrual: `Выручка без СПП` теперь является atom-метрикой: `SUM("Вайлдберриз реализовал Товар (Пр)")` только по строкам `Продажа`, без вычитания возвратов.
- WB accrual: ядро метрик разделено на чистые `metrics/atoms.ts`, простые комбинации `metrics/molecules.ts` и сложные итоговые `metrics/cells.ts`. Карта зависимостей вынесена в `docs/WILDBERRIES_METRICS_DEPENDENCY_MAP.md`.
- WB accrual: tooltip/formula для блоков `Итоги периода`, `Общие затраты по Маркетплейсу` и `Сруктура расчета` раскрывают полные формулы с CSV-колонками, включая подробное правило `net effect`.
